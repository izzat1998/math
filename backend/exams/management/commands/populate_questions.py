"""
Savollar bazasini olimpiada darajasidagi masalalar bilan to'ldirish (o'zbek tilida).
O'rtacha qiyinlik: 8/10. Universitet kirish imtihoni darajasi.
"""
...
QUESTIONS = [
    ...
    # ── GEOMETRIYA (6 ta, qiyinlik 7-9/10) ───────────────────────
    {
    ...
    # ── EHTIMOLLIK / KOMBINATORIKA (4 ta, qiyinlik 8-9/10) ───────
    {
    ...
    # ── SONLAR NAZARIYASI (5 ta, qiyinlik 7-9/10) ────────────────
    {
...
class Command(BaseCommand):
    help = "Savollar bazasini olimpiada darajasidagi masalalar bilan to'ldirish"

    def add_arguments(self, parser):
        parser.add_argument('--clear', action='store_true', help="Avval barcha savollarni o'chirish")
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(
            f"\n  Tayyor! {created} ta savol yaratildi, {skipped} tasi allaqachon mavjud edi.\n"
            f"  O'rtacha qiyinlik: 8/10 (universitet/olimpiada darajasi)\n"
            f"  Mavzular: {len([q for q in QUESTIONS if q['topic']=='algebra'])} algebra, "
            f"{len([q for q in QUESTIONS if q['topic']=='geometry'])} geometriya, "
            f"{len([q for q in QUESTIONS if q['topic']=='probability'])} ehtimollik, "
            f"{len([q for q in QUESTIONS if q['topic']=='number_theory'])} sonlar nazariyasi"
        ))
